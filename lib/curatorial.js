...
// Claves estables (se guardan en DB); las etiquetas públicas (bilingües)
// viven en messages → council.spec.<key>. Aquí el label en español para
// la consola interna del Founder.
export const SPECIALTIES = [
  { key: 'painting',       es: 'Pintura' },
  { key: 'sculpture',      es: 'Escultura' },
  { key: 'photography',    es: 'Fotografía' },
  { key: 'works_on_paper', es: 'Obra sobre papel' },
  { key: 'new_media',      es: 'Nuevos medios / digital' },
  { key: 'installation',   es: 'Instalación' },
  { key: 'criticism',      es: 'Crítica de arte' },
  { key: 'museum',         es: 'Curaduría de museo' },
  { key: 'market',         es: 'Mercado del arte' },
  { key: 'art_history',    es: 'Historia del arte' },
];
export const SPECIALTY_KEYS = SPECIALTIES.map((s) => s.key);
export function specialtyEs(key) {
  return SPECIALTIES.find((s) => s.key === key)?.es ?? key;
}

// Etapas del pipeline del Founder (orden = recorrido natural).
export const CANDIDATE_STAGES = [
  { value: 'new',       label: 'Nuevo',        cls: 'is-new' },
  { value: 'reviewing', label: 'En revisión',  cls: 'is-reviewing' },
  { value: 'interview', label: 'Entrevista',   cls: 'is-interview' },
  { value: 'approved',  label: 'Aprobado',     cls: 'is-approved' },
  { value: 'on_hold',   label: 'En pausa',     cls: 'is-hold' },
  { value: 'rejected',  label: 'Rechazado',    cls: 'is-rejected' },
];
export function stageLabel(value) {
  return CANDIDATE_STAGES.find((s) => s.value === value)?.label ?? value;
}
export function stageClass(value) {
  return CANDIDATE_STAGES.find((s) => s.value === value)?.cls ?? 'is-new';
}
